...
- DS/DSi raw NiFi and native 3DS LAN retain bounded discovery, ACK/retry, CRC, sequence, fragment, input-buffer, retransmission, state-hash, and initial SRAM checks.
...
- DS/DSi raw NiFi와 네이티브 3DS LAN은 제한 시간 검색, ACK/재전송, CRC, 순번, 분할, 입력 버퍼, 누락 입력 재전송, 상태 해시 및 초기 SRAM 검사를 유지합니다.
