...
Native 3DS rendering now composes a complete 160×144 frame away from the scanout buffers and presents it at the frame boundary. Off, aspect-preserving, and full-screen output sizes plus optional filtering are available on 3DS as well as DS/DSi. DS, DSi, and 3DS can select an exact 0x900-byte GBC BIOS file; saving settings records the absolute `biosfile` path in `gameyobds.ini`. No BIOS is included or distributed.
...
네이티브 3DS 렌더러는 160×144 완성 프레임을 화면 출력 버퍼 밖에서 구성한 뒤 프레임 경계에서 표시합니다. DS/DSi와 마찬가지로 3DS에서도 끄기·비율 유지·전체 화면 크기와 선택형 필터를 사용할 수 있습니다. DS·DSi·3DS 모두 정확히 0x900바이트인 GBC BIOS 파일을 선택할 수 있으며, 설정 저장 시 절대 `biosfile` 경로를 `gameyobds.ini`에 기록합니다. BIOS는 배포물에 포함하지 않습니다.
